...
async def test_record_parse_increments_counters(db_session: AsyncSession):
    ...
    metric = await svc.record_parse("airbnb", success=True, confidence=0.85)
    assert metric.ota_source == "airbnb"
    ...
    metric2 = await svc.record_parse("airbnb", success=False, confidence=0.0)
...
async def test_accuracy_calculation(db_session: AsyncSession):
    ...
    accuracy = await svc.get_accuracy("mmt", days=3)
    # last 3 days including today: 3 days * 10 parsed = 30 total, 21 successful
    assert accuracy == 0.7

    accuracy_all = await svc.get_accuracy("mmt", days=10)
...
async def test_metrics_endpoint_returns_metrics(client: AsyncClient, db_session: AsyncSession):
    svc = ParseMetricService(db_session)
    await svc.record_parse("goibibo", success=True, confidence=0.9)
    await svc.record_parse("goibibo", success=False, confidence=0.0)
    ...
    assert len(data) >= 1
    goibibo_metrics = [m for m in data if m["ota_source"] == "goibibo"]
...
async def test_accuracy_endpoint(client: AsyncClient, db_session: AsyncSession):
    ...
    response = await client.get(
        ...
        params={"ota_source": "airbnb", "days": 7},
        headers={"Authorization": f"Bearer {token}"},
    )
    assert response.status_code == 200
    data = response.json()
