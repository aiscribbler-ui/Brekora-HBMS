    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Overview of today's operations and pending tasks</p>
        </div>
        <button
          onClick={refresh}
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-5 md:col-span-2 lg:col-span-2 hover:shadow-md transition-shadow" aria-label="Properties list">
            ...
            {properties.length === 0 ? (
              <div className="py-6 text-center">
                <BuildingOfficeIcon className="h-10 w-10 text-gray-300 dark:text-gray-600 mx-auto mb-2" aria-hidden="true" />
                <p className="text-sm text-gray-500 dark:text-gray-400">No properties found.</p>
                <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Add your first property to get started</p>
              </div>
            ) : (
                ...
                {properties.map((p) => (
                  <li key={p.id} className="py-3 flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400">{p.address}</p>
                    </div>
                    <span
                    ...
                    >
                      <span
                        className={`w-1.5 h-1.5 rounded-full mr-1.5 ${
                          p.status === 'active' ? 'bg-success' : 'bg-gray-400 dark:bg-gray-500'
                        }`}
                        aria-hidden="true"
                      />
                      {p.status}
