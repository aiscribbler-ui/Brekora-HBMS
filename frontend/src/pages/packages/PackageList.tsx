    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 rounded-md border border-gray-300 text-sm focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500"
          />
        </div>
        <select
          value={statusFilter}
          onChange={(e) => setStatusFilter(e.target.value as StatusFilter)}
          className="w-full sm:w-40 rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500"
        >
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-white rounded-lg border border-gray-200">
          <ArchiveBoxIcon className="mx-auto h-10 w-10 text-gray-300" />
          <p className="mt-3 text-sm text-gray-500">No packages found.</p>
          {search && (
            <button
              onClick={() => setSearch('')}
              ...
              <tbody className="divide-y divide-gray-200">
                {filtered.map((pkg) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-sm text-gray-600">₹{pkg.base_price}</td>
                    <td className="px-4 py-3 text-sm text-gray-600">{compositionSummary(pkg)}</td>
          ...
          <div className="md:hidden space-y-3">
            {filtered.map((pkg) => (
              ...
              >
                ...
                <p className="mt-1 text-xs text-gray-500">{compositionSummary(pkg)}</p>
                <p className="mt-1 text-xs text-gray-600 font-medium">₹{pkg.base_price}</p>
              </button>
            ))}
          </div>
        </>
