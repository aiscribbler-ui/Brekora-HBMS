    ...
    <div className="space-y-4">
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <label htmlFor={`addon-${addon.id}`} className="block text-sm font-medium text-gray-900 cursor-pointer">
                      {addon.name}
                      <span className="ml-2 text-xs text-gray-500">₹{addon.unit_price}</span>
                    </label>
                    {addon.description && (
                    ...
                    {selected && (
                      <div className="mt-3 flex flex-col sm:flex-row gap-3">
                        ...
                        <div className="flex items-center gap-2 pt-5">
                          ...
                          />
                          <label htmlFor={`addon-included-${addon.id}`} className="text-sm text-gray-700">
                            Included in package
                          </label>
                        </div>
                      </div>
      ...
      <div className="bg-gray-50 rounded-lg border border-gray-200 p-4">
        <div className="text-sm text-gray-600">
          ...
          <span className="font-semibold text-gray-900">₹{estimatedAddOnCost.toFixed(2)}</span>
          {selectedAddOns.some((s) => s.is_included) && (
            <span className="text-xs text-gray-500 ml-2">(some included)</span>
          )}
        </div>
      </div>
    </div>
